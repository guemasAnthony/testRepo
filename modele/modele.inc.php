 ...
 */
   ...
   public function getInfosVisiteur($login, $mdp)
   {
       // retourne les informations sur le visiteur (la table utilisateur)
         $req = "SELECT id, nom, prenom FROM utilisateur WHERE login='".$login."' AND mdp='".$mdp."'";
   ...
   public function getLesProjets() {
    ...
    public function getLesProjetsDetail() 
    {
     // retourne un tableau associatif contenant les projets et les employes associes
         $req="SELECT p.codeprojet, nomprojet, nom, prenom FROM projet p, employeinformaticien e WHERE e.codeprojet = p.codeprojet ORDER BY nom";
         $rs = PdoExemple::$monPdo->query($req);
		$ligne = $rs->fetchAll(PDO::FETCH_ASSOC);
		return $ligne;
    }
    ...
    public function verifierLog($login, $mdp)
    {
